...
This object serves double duty as the context for the command and the application itself. There are some additional options that control the application's behavior. The first is `process.exit()` which allows Stricli to set the exit code of the application once the command finishes (or throws an error). The other is `locale` which is used by the [localization logic](./configuration.mdx#localization-localization) to determine which language the text should be in.
...
The `forCommand` method receives a [`CommandInfo`](../../packages/core/type-aliases/CommandInfo) object, which contains:

-   `prefix`: The command path as an array of strings (e.g., `["db", "query"]` for `run db query name`)

This object could potentially contain other contextual information about the matched command, but currently it is only `prefix`. If you have a use case that requires additional contextual information passed from Stricli to your command, please [open an issue on GitHub](https://github.com/bloomberg/stricli/issues/new/choose).
